...
function HERODEFAULT:InitializeClass(hero)
	...
	function hero:OnRespawn()
		...
		local invuln = ParticleManager:CreateParticle("particles/items3_fx/lotus_orb_shield.vpcf", PATTACH_CUSTOMORIGIN_FOLLOW, hero)
	...
	function hero:OnSwitchWeapon(slot)
		...
		if hero.activeWeapons[slot] then
			...
			FireGameEvent("weapon_change", {pid=hero:GetPlayerID(), weapon=hero.weapon.name, time=hero.weapon.switchTime})
	...
	function hero:OnReloadWeapon()
		...
		if hero.weapon.usesAmmo and hero.weapon.isReloadable and hero.weapon.ammo ~= hero.weapon.ammoCapacity and hero.weapon.ammoReserve > 0 
			...
			FireGameEvent("weapon_reload", {pid=hero:GetPlayerID(), time=hero.weapon.reloadTime})
