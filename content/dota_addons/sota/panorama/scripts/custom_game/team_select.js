...
{
	$.Msg($("#ScoreSetting").GetSelected().text);
	GameEvents.SendCustomGameEventToServer( "Sota_Set_Setting", {name: "score_max", value: parseInt($("#ScoreSetting").GetSelected().text)});
}
...
{
	var playerInfo = Game.GetLocalPlayerInfo();
	if (playerInfo && playerInfo.player_has_host_privileges){
		$("#ScoreSetting").enabled = true;
	}
	else{
		var scoreMax = CustomNetTables.GetTableValue( "sotaui", "score_max" );
		$("#ScoreSetting").SetSelected("s" + scoreMax.value);
	}

	$.Schedule(.1, CheckHost)
}
